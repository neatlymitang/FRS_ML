...
function [bestacc,besto,bestg,feature,cg,X,Y] = MLFRScgForClass_w2(train_data,train_target,test_data,test_target,omin,omax,gmin,gmax,ostep,gstep)
...
%         feature_slct=fuzzy_pes(train_data,train_target,cmd);
         feature_slct=fuzzy_w_opt(train_data,train_target,cmd);
...
function [feature_slct,feature_dependfirst,feature_dependlast]=fuzzy_w_opt(data,target,cmd)
tic
...
%             kernel_matrix=or-dis;
    ...
    TTopt{1}=[TTopt{1},terminal];
    
%     if terminal<0.004|num_cur==0%一个终止条件
